...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (args.size() != 1) {
    throw Exception(fmt::format("func call {} length of args should not be {}", func_name, args.size()));
  }

  auto tp = func_name == "lower" ? StringExpressionType::Lower : StringExpressionType::Upper;

  return std::make_shared<StringExpression>(args.at(0), tp);
}
